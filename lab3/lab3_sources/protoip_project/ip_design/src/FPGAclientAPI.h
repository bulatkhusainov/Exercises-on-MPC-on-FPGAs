...
int write_stimuli(double input_data[], unsigned packet_internal_ID)
{
	...
	{
	case 0: //x_hat_in
		// store x_hat_in into ../../ip_design/test/stimuli/my_project0/x_hat_in.dat
		pFile = fopen ("../../ip_design/test/stimuli/my_project0/x_hat_in.dat","w+");

		for (i = 0; i < X_HAT_IN_LENGTH; i++)
		{
			fprintf(pFile,"%2.18f \n",input_data[i]);
		}
		fprintf(pFile,"\n");
...
{
	system("vivado_hls -f ../../.metadata/my_project0_ip_design_test.tcl");
}
...
int read_results(double output_data[], unsigned packet_internal_ID, unsigned packet_output_size)
{
	...
	{
	case 0: //u_opt_in
		// load ../../ip_design/test/results/my_project0/u_opt_out.dat
		pFile = fopen ("../../ip_design/test/results/my_project0/u_opt_out.dat","r");
